...
//\                                                                   /\\
//\  Brief   : Controls health bar for West and East doors.           /\\
//\                                                                   /\\
//\=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=/\\
...
public class HealthBarScriptWE : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (gate)
        {
           vPosition.y = 1.1f * ((float)gate.GetComponent<GateScript>().iHealth / (float)gate.GetComponent<GateScript>().iMaxHealth - 1.0f);
